...
/* Global */
pcl::PointCloud<pcl::PointXYZI>::Ptr currentPC(new pcl::PointCloud<pcl::PointXYZI>());
...
{
  std::lock_guard<std::mutex> guard(mutex); //unique_lock是write lock。被锁后不允许其他线程执行被shared_lock或unique_lock的代码。
...
int main(int argc, char** argv) 
{
  ...
  ros::NodeHandle nh;
  ros::Subscriber sub = nh.subscribe<sensor_msgs::PointCloud2>("/point_cloud_raw", 1, callback); 
  ros::Publisher pub = nh.advertise<nav_msgs::OccupancyGrid>("/occu_map", 1);
